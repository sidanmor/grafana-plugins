...
test('should display series counter when "Show series counter" option is enabled', async ({
  ...
  await panelEditPage.setVisualization('timeseriesx');
  await panelEditPage.collapseSection('timeseriesx');
  await panelEditPage.refreshPanel();
  await expect(page.getByTestId('simple-panel-series-counter')).not.toBeVisible();
  const showSeriesSwitch = panelEditPage
    .getByGrafanaSelector(selectors.components.PanelEditor.OptionsPane.fieldLabel('timeseriesx Show series counter'))
